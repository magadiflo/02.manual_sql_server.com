...
Sobre los objetos de base de datos se pueden administrar los permisos de forma explícita para que 
los usuarios accedan a ellos. (Ver Logins) (Ver Usuarios de base de datos)

Cada asegurable tiene permisos que se pueden otorgar a una entidad de seguridad mediante la instrucción
de permiso Grant (Ver Asignar permisos con Grant) y para quitar los permisos se debe utilizar Deny.

Los siguientes párrafos corresponden a la explicación del artículo donde se asignan los permisos con 
...
mínimos e inician sesión con cuentas de usuario limitadas ya sea en base a roles o de manera 
...
Es más sencillo agregar o quitar usuarios de base de datis de un rol que volver a crear conjuntos 
...
El encapsulamiento del acceso a los datos a través de módulos tales como procedimientos almacenados 
y funciones definidas por el usuario brinda un nivel de protección adicional a la aplicación.
...
procedimientos almacenados. Si el usuario logra conectarse a SQL Server no verá ningún objeto
pero si podrá ejecutar los procedimientos almacenados a los que se le ha asignado permiso de ejecución.
...
Donde
All Opción que se mantienen por compatibilidad con versiones anteriores.
...
Si el asegurable es función que retorna una tabla, ALL asigna DELETE, INSERT, REFERENCES, SELECT y UPDATE.
...
To principal: el principal que se le denegarán los permisos.

Para hacer los ejercicios se incluirá sintaxis separadas
...
3. Deny con Vista, Tabla, Sinónimo o función definida por el usuario
...
Denegar el permiso de Eliminación.*/
use master
go
...
AdventureWorks que tengo permisos de Lectura, modificación, inserción en el esquema Sales (Ventas)
Denegar Eliminación.*/
use master
go
...
Crear un usuario llamado Contable que tenga acceso sólo a los esquemas Person y HumanResorces, 
