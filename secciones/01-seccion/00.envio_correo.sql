USE [si_solm]
GO
/****** Object:  StoredProcedure [dbo].[USP_NOTIFICACION_IBP_X_VENCER]    Script Date: 22/04/2021 10:33:37 a.m. ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE[dbo].[USP_NOTIFICACION_EMO_X_VENCER](@PERS_TIPOTRAB VARCHAR(2),@TIPO_CORREO INT)  AS
BEGIN

DECLARE  @CODIGO_SUCU VARCHAR(2),@AUX INT, @AUX2 INT, @I INT,@ADMINISTRADOR VARCHAR(300),@ASUNTO VARCHAR(300),@SUCURSAL_NOMBRE VARCHAR(50), @PERIODO VARCHAR(50), @CORREO_ADMI VARCHAR(300), @CORREO_JEOPE VARCHAR(300), @CORREOS_CC VARCHAR(800),@MENSAJE VARCHAR(1000)
DECLARE @FECHA DATETIME ,@JEFERRHH VARCHAR(300),@USUARIO VARCHAR(50),@ETIQUETA VARCHAR(50), @TITULO VARCHAR(50),@BANNER VARCHAR(100),@VIGENCIA_PERS VARCHAR(2), @DIRECTORIO VARCHAR(MAX)
			IF @TIPO_CORREO=1 BEGIN
				
				SET @FECHA =  CAST(CONVERT(CHAR(10), DATEADD(DAY,(15), GETDATE()), 103) AS DATETIME)
			  --SET @FECHA = CAST(CONVERT(CHAR(10), DATEADD(DAY,(30), '11/12/2022'), 103) AS DATETIME)
				SET @VIGENCIA_PERS ='SI'
			END ELSE IF @TIPO_CORREO=2 BEGIN
		...
		--WHERE lds_correo = 'rrhh@gruposolmar.com.pe'  and isnull(lds_correo_principal,0) = 1 ;
				...
				DECLARE @TABLA TABLE(IT INT IDENTITY(1, 1),CODI_SUCU VARCHAR(2),EMPRESA VARCHAR(100),SUCURSAL VARCHAR(40),TIPO VARCHAR(20),CODIPERS CHAR(5),NOMBRES VARCHAR(150),DNI VARCHAR(15),CARGO VARCHAR(150),CERT_FECHA_EMISION CHAR(10),FECH_INGRE_PLANILLA CHAR(10),FECHAINI CHAR(10),FECHAFIN CHAR(10),CLIENTE VARCHAR(100),USUA_REG VARCHAR(50),CERT_VIGENCIA VARCHAR(50), SUCU_ABREVIATURA VARCHAR(30))
				INSERT INTO @TABLA
				...
				WHERE CONT_VIGENCIA = 'SI' 
					...
					AND CERT_VIGENCIA IN ( SELECT  TOP 1 RE.CERT_VIGENCIA FROM REDO_CERTIFICADO RE WHERE RE.REQU_CODIGO = 81 AND RE.CODI_PERS = R.CODI_PERS  AND RE.CERT_VIGENCIA=@VIGENCIA_PERS ORDER BY RE.CERT_CODIGO DESC )
					--
					AND ( SELECT  TOP 1 RE.CERT_VIGENCIA FROM REDO_CERTIFICADO RE WHERE RE.REQU_CODIGO = 81 AND RE.CODI_PERS = R.CODI_PERS ORDER BY RE.CERT_CODIGO DESC )= @VIGENCIA_PERS
					--
...
SELECT * FROM @TABLA	
	...
	IF @AUX2 >0 BEGIN 
		DECLARE @ADMIN TABLE(ID INT IDENTITY (1, 1), CODIGO VARCHAR(5), NOMBRES VARCHAR(300),NUMDOCU VARCHAR(10),CARGO VARCHAR(150), ETIQUETA VARCHAR(150),CORREO VARCHAR(150),  CORREO_ADMI varchar(500),LOGIN varchar(20) )
		INSERT INTO @ADMIN
			SELECT	P.CODI_PERS,
			...
			WHERE    (P.codi_pers =(select codi_pers FROM  Intranet.[dbo].[JEOSIS_Libreta_Direcciones_SOLMAR] where lds_id in ('634'))COLLATE Modern_Spanish_CS_AS)
			--P.codi_pers in('11424')--(09852)
		...
		WHILE @AUX>=@I BEGIN
           ...
           SET @CORREO_ADMI=(SELECT CORREO_ADMI FROM @ADMIN WHERE ID=@I)
			...
			IF @TIPO_CORREO=1 BEGIN
					...
					SET @ASUNTO = 'IBP03: IBP Próximos a vencer'
					SET @TITULO ='PERSONAL CON IBP PRÓXIMO A VENCER'
					SET @DIRECTORIO ='\\192.168.10.2\Biblioteca_Grafica\TEXTO\Firma\SECURITY_1.png;\\192.168.10.2\Biblioteca_Grafica\TEXTO\Sublogos\Grupo-SECURITY.png;\\192.168.10.2\Biblioteca_Grafica\TEXTO\Firma\RRHH_IBP_PROXIMO_VENCER.jpg'
			...
			END ELSE IF @TIPO_CORREO=2 BEGIN
					...
					SET @ASUNTO = 'IBP04: IBP Vencidos'
					SET @TITULO ='PERSONAL CON IBP VENCIDO'
					SET @DIRECTORIO ='\\192.168.10.2\Biblioteca_Grafica\TEXTO\Firma\SECURITY_1.png;\\192.168.10.2\Biblioteca_Grafica\TEXTO\Sublogos\Grupo-SECURITY.png;\\192.168.10.2\Biblioteca_Grafica\TEXTO\Firma\RRHH_IBP_VENCIDO.jpg'
			...
			-- SOLMAR Legal <legal@gruposolmar.com.pe>;448
			-- SOLMAR Recursos Humanos <rrhh@gruposolmar.com.pe>;462
			-- Miguel VIVANCO <miguelvivanco@gruposolmar.com.pe>; 499
			--SOLMAR Finanzas <finanzas@gruposolmar.com.pe>; 445
			--SOLMAR Logística <logistica@gruposolmar.com.pe>; 449
			--SOLMAR Contabilidad <contabilidad@gruposolmar.com.pe>;440
			--SOLMAR Operaciones  <operaciones@gruposolmar.com.pe>; 433
			...
			--SOLMAR Control Administrativo <controladmon@gruposolmar.com.pe>;757
			--SOLMAR Administracion <administracion@gruposolmar.com.pe>; 335 
			--SOLMAR Proyectos SW <proyectossw@gruposolmar.com.pe>;631 
			--SOLMAR Sistemas <sistemas@gruposolmar.com.pe>; 464

			
				DECLARE  @AUXCORREO VARCHAR(2)
				SELECT @AUXCORREO = COUNT(*) FROM  Intranet.[dbo].[JEOSIS_Libreta_Direcciones_SOLMAR]
				WHERE  (@TIPO_CORREO=1 and lds_id in ('462','335','582')) or(@TIPO_CORREO=2 and lds_id in ('462','335','582','501','499'/*,'448','757'*/))
				...
				DECLARE @TABLACORREO TABLE(lds_id INT IDENTITY(1, 1),lds_correos VARCHAR(200))
						...
						SELECT  lds_etiqueta +' '+'<'+  lds_correo + '>; ' AS Nombres FROM  Intranet.[dbo].[JEOSIS_Libreta_Direcciones_SOLMAR]
							WHERE (@TIPO_CORREO=1 and lds_id in ('462','335','582')) or(@TIPO_CORREO=2 and lds_id in ('462','335','582','501','499'/*,'448','757'*/))
				SET @LoopCounter =1
				...
				WHILE(@LoopCounter <= @MaxEmployeeId)
					BEGIN
						   ...
						   FROM  @TABLACORREO WHERE lds_id = @LoopCounter
						   SET @LoopCounter  = @LoopCounter  + 1 
						
					END
				

			  SET @CORREOS_CC = @l_correo 
			  select @CORREOS_CC
			  --+' '+'SOLMAR Administracion <administracion@gruposolmar.com.pe>;SOLMAR Control Administrativo <controladmon@gruposolmar.com.pe>'
			--SET @CORREOS_CC = 'SOLMAR Sisolmar  <sisolmar@gruposolmar.com.pe>;'+@CORREO_JEOPE
			
			--CONDICION CUANDO TENGA REGISTROS.
			IF @CANTIDAD_C>0 BEGIN
					...
					IF @TIPO_CORREO=1 BEGIN
						...
						'<p style="font-size:14px; font-family:Calibri;"><strong>Atentamente,</strong></p>' +
								...
								'<tr>' +
								'<td colspan="2">' +
								'<img src="Grupo-SECURITY.png" height="64" alt="SOLMAR2">' +
								'</tr>' +
								...
								'<strong><em><u><a href="mailto:mejoracontinua@gruposolmar.com.pe" target="_blank">mejoracontinua@gruposolmar.com.pe</a></u></em></strong>' +
								'</td></tr></strong>' +
								'</table></body></html>'
					END ELSE IF @TIPO_CORREO=2 BEGIN
						SET @tableHTML =
					...
					END
			--select @tableHTML,@ADMINISTRADOR,@ETIQUETA
			--select @CORREOS_CC
			...
			@blind_copy_recipients = 'SOLMAR Sistemas <sistemas@gruposolmar.com.pe>; SOLMAR SW Training <swtraining@gruposolmar.com.pe> ;SOLMAR Sisolmar <sisolmar@gruposolmar.com.pe>',
			@subject = @ASUNTO,
			@body = @tableHTML,
			@importance ='HIGH',
			@body_format = 'HTML',
